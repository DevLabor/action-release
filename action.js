...
async function upload (release, files, created) {
  var file = files.pop()
  ...
  await upload(release, files, created)
}

(async () => {
  try {
    api = new github.GitHub(core.getInput('token'))
    const tag = core.getInput('tag')
    let name = core.getInput('name')
    const body = core.getInput('body')
    verbose = core.getInput('verbose') === 'true' // input is always string, not boolean.
